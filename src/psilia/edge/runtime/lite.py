...
Sensor identity is not auto-resolvable here. The cv2/AVFoundation probe
returns a cv_index and a resolution, but no USB descriptors (vendor/
product/serial), so we can't fingerprint the device against the
`sensors:` registry in `psilia.yaml` the way the two-layer runtime does
(design.md → "Calibration Resolution"). Consequence: associating a
recording with a registered sensor — and therefore picking a calibration
file from `~/.psilia/calibrations/` — has to be a manual UI step. The
recording dir is the right place to snapshot the chosen calibration
(design.md → "Lite Mode → Recordings").
...
from psilia.edge.utils import read_yaml, write_yaml
...
from psilia.vision.camera_stream import CameraStream
from psilia.edge.runtime.config import get_web_dir
...
class _LiteState:
    def __init__(self) -> None:
        ...
        self.recording_ts_fp = None
        self.recording_path: Path | None = None
        self.recording_frames: int = 0
        self.recording_started: float = 0.0
        self.recording_t_start: float | None = None
        self.recording_t_end: float | None = None
    ...
    def on_frame(self, t: float, frame) -> None:
        ...
        with self.lock:
            ...
            if self.recording_fp is not None:
                seq = self.recording_frames
                self.recording_fp.write(jpeg)
                self.recording_frames += 1
                if self.recording_ts_fp is not None:
                    self.recording_ts_fp.write(
                        json.dumps({"seq": seq, "t_capture": t, "bytes": len(jpeg)})
                        + "\n"
                    )
                if self.recording_t_start is None:
                    self.recording_t_start = t
                self.recording_t_end = t
...
def api_cameras() -> JSONResponse:
    from psilia.edge.runtime.hotplug import probe_cv2_cameras
...
def api_record_start(session: str | None = None) -> JSONResponse:
    ...
    with _state.lock:
        ...
        ts_path = rec_dir / "timestamps.jsonl"
        _state.recording_fp = open(path, "wb")
        _state.recording_ts_fp = open(ts_path, "w")
        _state.recording_path = path
        _state.recording_frames = 0
        _state.recording_started = time.time()
        _state.recording_t_start = None
        _state.recording_t_end = None
...
def api_record_stop() -> JSONResponse:
    with _state.lock:
        ...
        t_start = _state.recording_t_start
        t_end = _state.recording_t_end
        _state.recording_fp.close()
        _state.recording_fp = None
        if _state.recording_ts_fp is not None:
            _state.recording_ts_fp.close()
            _state.recording_ts_fp = None
...
def api_recording_stream(name: str) -> StreamingResponse:
    ...
    ts_path = path.parent / "timestamps.jsonl"
        ...
        def gen():
            ...
            with open(ts_path) as f:
                for line in f:
                    ...
                    nbytes = rec["bytes"]
                    t_cap = rec["t_capture"]
                    jpeg = data[offset : offset + nbytes]
                    offset += nbytes
                    if t0 is None:
                        t0 = t_cap
                    sleep_t = (wall_start + (t_cap - t0)) - time.monotonic()
                    if sleep_t > 0:
                        time.sleep(sleep_t)
                    yield _multipart(jpeg)
    else:
        # Legacy recordings: no sidecar, scan SOI markers and pace by metadata fps.
        fps = 30.0
        if meta_path.exists():
            try:
