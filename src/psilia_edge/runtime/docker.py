...
def start_container(image: str, ros_workspace: str) -> dict:
    ...
    if status == "running":
        return {"status": "already_running", "container": CONTAINER_NAME, "image": image}

    if status == "exited":
        rc, _, err = _run(["docker", "rm", CONTAINER_NAME])
        if rc != 0:
            return {"status": "error", "error": f"Failed to remove stale container: {err}"}

    rc, _, err = _run([
        "docker", "run", "-d",
        "--name", CONTAINER_NAME,
        "--network", "host",
        "-v", f"{ros_workspace}:/opt/psilia/ros",
        image,
    ])
    if rc != 0:
