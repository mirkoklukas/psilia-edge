...
- Before launch, `start_spatial_layer()` runs requirement checks (container, camera, calibration, resolution, hotspot) and writes `~/.psilia/run/launch_params.yaml` and `~/.psilia/run/launch_id`. The `launch_id` file (`{id}:{timestamp}`) is the liveness token for the spatial layer — `core_node` echoes the same id in every `heartbeat.json`, so the host can detect state (stopped / running / stale / crashed) with cheap file reads. `stop_spatial_layer()` appends `:stopped` to the file. This file contains a `nodes` list of which spatial nodes to start (e.g. `camera_node`, `rectify_node`) and per-node parameters (device path, calibration file, resolution). After building the requirement-derived node list, user overrides from `runtime.yaml` `ros.nodes` are applied — nodes set to `false` are removed. The launch file (`default.launch.py`) always starts infrastructure nodes (core, ping, recording, rosbridge, rosapi) and only adds the nodes listed in `launch_params.yaml`.
